...
const MyFavourite = () => {
    ...
    const handleUnfav=(artworkId)=>{
        console.log(artworkId);
        fetch(`http://localhost:3000/favourite/${artworkId}`,{
            method: 'DELETE'
        })
        .then(res =>res.json())
        .then(data=>{
            ...
            if(data.deletdConunt>0){
                toast.success('Remove from Favourite');
                const filter = data.filter(fav => fav._id !== artworkId)
                setFavourite(filter)
            }
    ...
    <div className="container mx-auto px-4 py-12">
      <h2 className="text-3xl md:text-4xl font-bold text-center mb-10 text-gray-800">
        My <span className="text-indigo-600">Favourite</span> Artworks
      </h2>
          ...
          {favourite.map((item) => (
            ...
            >
              ...
              <div className="text-center space-y-2">
                ...
                <button onClick={()=>handleUnfav(item.artworkId)}
                  className="
